REM This script has been created only for programming learning purposes:

REM This is a simple script that restores MS Office. 
REM My internet conection isn't that fast. So, I don't want to redownload the same program several times.

REM --------------------------------------------------------------------------------------------------------
REM Backup and Restore Script Sintax:

REM @echo off
REM robocopy [source][destination] /e(everything) /eta(estimated time)
REM If the directory contains spaces, use the "" to tell the script interpreter to read the directory path as STRING!

REM Exemple: 

REM robocopy [source][destination] /e(everything) /eta(estimated time)
REM robocopy "C:\path" "C:\path" /e /eta
REM exit
REM --------------------------------------------------------------------------------------------------------


REM Restoring files to its proper folder:
@echo off
robocopy "C:\Users\felip\Desktop\Office_2021_Preview_Full_Backup\Office_2021_Preview_Files" "C:\Program Files\Microsoft Office" /e /eta
exit
